...
任何旧审核结论都不直接作为恢复后的通过凭据。提交和 DONE 必须绑定新的审核包、准确的 Git tree 与完整测试集。报告文件、HEAD、退出码或租约到期各自都不能授权 DONE。
...
2. `review_packages`：不可变 package ID 绑定完成的 check run、当时的 route 与执行 attempt、审核前 Git 身份和有界完整 diff。审核 attempt/result 必须引用 package ID；审核返回后重测同一树，再在一个事务中结束 attempt、保存 verdict，并按通过或返工进入下一状态。旧 verdict 不直接授权崩溃后的提交；恢复时重跑 Codex review。
...
commit 完成且再次核验后，以固定输入构造 report operation，先在 SQLite 保存报告 JSON、diff 字节和各自哈希。写临时文件、校验、rename 后复读目标文件并核对哈希；若文件缺失或不符，重启时只能从已保存的相同字节重写，不能依据现有文件推断完成。两个产物都核对后写 report-complete marker。
...
1. 增加 check run / review package 的不可变绑定，并让 `running → reviewing` 与 package 创建在同一事务完成。先开放 `reviewing` 中断后的重新检查与新 Codex review；任何旧 verdict 失效。检查定义或树变化时隔离，不复用部分结果。
...
本协议首先覆盖进程崩溃。断电和操作系统故障还需要核验 SQLite 同步级别、Git 对象及引用落盘策略、报告 rename 的文件与目录持久性；完成这些验收前不声称具备断电级的 exactly-once 保证。
