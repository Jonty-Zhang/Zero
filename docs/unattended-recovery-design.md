...
- 当前服务需要进程常驻，没有在目标电脑安装开机启动、守护和单实例机制。
...
1. **进程包含。** Windows 执行进程及其后代必须进入 `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` 的 Job Object，并在任务代码开始前完成分配。[微软的 Job Object 文档](https://learn.microsoft.com/en-us/windows/win32/procthread/job-objects)说明最后一个 Job 句柄关闭时会结束关联进程，子进程默认继承 Job，且可禁止 breakaway。Zero 进程异常退出时，持有 Job 的监督者必须关闭句柄并等待进程树退出。监督者同时监听 Zero 父进程的生命期；仅让一个独立子进程持有 Job 句柄而不监听父进程，不构成崩溃清理。Windows 实现宜由随应用打包的原生进程监督层承担；Node 的 `taskkill` 只作活进程取消路径。非 Windows 平台使用独立进程组并提供等价的退出确认。
...
3. **启动协调。** 数据目录只能有一个活动 Zero 服务实例。新实例获取排他锁，确认上一次监督层已经结束且没有任务进程存活，再运行恢复扫描。不能在旧服务仍运行时直接 `recoverExpired` 并重复派工。
...
5. **启动身份。** Codex 订阅与 ZCode 桌面提供方依赖用户身份、网络和本机配置。Windows Task Scheduler 的 `InteractiveToken` 需要用户已登录；`S4U` 不保存密码，但[微软文档明确其无法访问网络或加密文件](https://learn.microsoft.com/en-us/windows/win32/taskschd/taskschedulerschema-logontype-principaltype-element)，不适合作为这里的默认方案。真正的无登录开机运行需在同一用户安全上下文下配置受控的凭据启动，并分别验证 Codex 订阅、代理、ZCode 提供方和工作目录。安装器应提供可检查的任务定义，用户同意后才注册系统任务；应用不写入用户现有 ZCode 配置。
