...
本方案使用 Windows Task Scheduler 的系统启动触发器和重复 watchdog 触发器运行 Zero 服务。安装脚本默认只显示配置并退出；只有明确传入 `-Install` 才会注册任务。安装需要显式提供已构建的原生 guardian 可执行文件绝对路径。卸载仅移除计划任务，保留数据库、配置、worktree、报告和日志。
...
脚本先检查任务名不存在、Node 版本、账户 SID、管理员组和目录位置，再通过系统凭据对话框取得密码并注册一个有限权限的计划任务。它不会立即启动服务。任务包含系统启动触发器，以及从注册时约五分钟后开始、每五分钟重复一次的 watchdog 触发器。watchdog 与有限的快速失败重启配合：Zero 仍在运行时，`IgnoreNew` 会忽略重复触发；进程干净退出或三次快速重启用尽后，后续 tick 可再次启动任务。它只在任务进程不再运行时补启动，不检测仍存活但卡住的服务。默认配置如下：
...
| 设置 | 默认值或行为 |
|---|---|
| 触发器 | 系统启动；另有约五分钟后开始、每五分钟重复的 watchdog |
...
| 多实例策略 | `IgnoreNew`；watchdog tick 不会并行启动第二个仍运行的任务 |
| 失败重启 | 每次失败后间隔 1 分钟，最多 3 次；后续 watchdog tick 可再尝试启动 |
...
之后重启电脑，在不登录该账户的情况下再次检查健康地址和日期日志，确认启动触发器、watchdog 和非交互凭据均有效。还应在测试环境分别验证服务进程退出、快速重启次数用尽后的恢复，以及进程仍运行但健康检查失败时 watchdog 不会重复拉起。再提交一个低风险任务，检查它完成了检查、Codex 审核、返工策略和报告归档。当前脚本注册逻辑和触发器配置已有自动化/CI 检查，但目标电脑上的计划任务注册、启动、重复崩溃恢复和重启验收仍未完成；开始无人值守运行前，确保任务仓库可信，任务账户没有管理员权限，并为日志和 worktree 预留磁盘空间。
