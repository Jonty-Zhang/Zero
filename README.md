...
- A `waiting` state for verified provider usage limits, with a persisted checkpoint and scheduled retry. Quota resume is a separate checkpoint-based path and is mock-tested across service restart; a real provider quota event has not been observed. For ordinary crashes, Zero first quarantines expired leases. After the native guardian verifies that the immediately preceding process Job is drained, a narrow recovery path can resume an eligible task in the same worktree: it takes a fresh baseline, chooses a new route, starts a new execution attempt, then reruns every configured check and Codex review. Eligibility is limited to a `running` task at revision 0 with at most one execution stage and no review, commit, or report phase. Repeated crashes must preserve the verified generation lineage. An interrupted review, an unreviewed local commit, missing lineage, or any worktree identity/path mismatch remains `recovery_required` for inspection.
