...
Zero is a local-first task execution node for coding work. Submit a task through the local UI or CLI; Zero stores it in a persistent queue, routes it, runs it in a Git worktree, performs configured checks, requests a separate Codex review, and archives the result. On Windows, start the long-running local service manually from the Start Menu shortcut or launcher command. It keeps working while the launcher remains open, even when the browser is closed.
...
- Recovery is implemented with automated test coverage; target-PC validation of the Windows manual launcher and recovery, real Harness execution, provider quota recovery, and crash fault injection remains outstanding. Ignored build/cache files are not part of Zero's Git-based worktree fingerprint; they can remain in the worktree and affect commands after recovery, so rely on reproducible checks rather than hidden local cache state. See the [crash recovery design](docs/crash-recovery-design.md) and [recovery implementation boundary](docs/crash-recovery-next-slice.md).
...
- A native Windows process guardian with passing CI build and process-containment tests. The user starts the supervised service manually; the launcher retries nonzero exits with backoff, and the user can relaunch it after stopping it or restarting Windows. Deployment and recovery on the target machine remain unverified. See [Windows deployment](docs/windows-deployment.md).
- A Windows release staging script bundles the built runtime, UI, CLI, explicitly supplied Node executable, and tested guardian with a file-hash manifest. An independent verifier checks every file and starts the bundled CLI in CI. An unsigned NSIS installer is built and passes install/uninstall smoke tests on GitHub's Windows runner; it provides manual Start Zero and dashboard shortcuts. Installation and a real Harness task on the target machine remain unverified; see [Windows app packaging](docs/windows-app-packaging.md).
...
Replace the repository and binding placeholders with real values. Every manually selected Harness/model/effort combination must be currently available and verified. Steps are kept in the submitted order. For steps in the same repository, later work starts from the earlier step's verified result commit. When goal metadata is present, Zero runs an aggregate Codex review after all steps finish and includes its current state and result in the sequence API and UI. A `PASS` yields `completed`; `changes_requested` may start an automatic aggregate-goal remediation task, bounded by `maxGoalRevisions`, while `blocked` keeps the sequence blocked and quota pauses it in `waiting`. `maxGoalRevisions` is separate from each task's `maxRevisions`. The live end-to-end goal-review and remediation flow remains unverified.
...
The HTTP service binds to `127.0.0.1:4179` by default. v1 does not support remote binding. A Git worktree separates task changes from the main checkout, but it is **not an operating-system security sandbox**. The Zero service and validation commands run as the Windows user who starts the launcher, so use a suitably low-privilege account with access limited to intended repositories and credentials. Validation commands run as child processes and should be treated as code execution.
...
- [Windows manual deployment](docs/windows-deployment.md)
