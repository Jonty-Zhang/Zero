...
- `POST /api/tasks`：接收 `{ repoPath, baseRef, prompt, acceptanceCriteria, maxRevisions, checkCommands, execution: { harnessId, modelId, reasoningEffort } }`。手动项可为 `null`。服务端可映射为领域层 `selection.harness/model/reasoningEffort` 和 `checks`。
